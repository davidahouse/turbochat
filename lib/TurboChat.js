...
function TurboChat(configuration) {
  ...
  this.colloquy = new TurboChatColloquy(configuration["colloquies"]);

  // this.name = configuration["name"];
  // this.knowledge = configuration["knowledge"];
  // this.actions = configuration["actions"];
  // this.attributes = configuration["attributes"];
  this.processor = new TurboChatProcessor(this.colloquy);
...
TurboChat.prototype.start = function() {
  ...
  this.interface.listen(function(speech,message) {
    ...
    var dialog = this.colloquy.dialogForName(guess);
    if (dialog != null) {

      if (dialog["callback"]) {
        var botResponse = this.prototype[dialog["callback"]](message.text);
        if (botResponse) {
          this.interface.reply(message, botResponse);
        }
      }
      this.interface.reply(message, dialog["response"]);
    } else {

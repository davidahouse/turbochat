...
TurboChatProcessor.prototype.teach = function() {
  ...
  for (var action in this.actions) {
    this.classifier.addDocument(this.actions[action], this.actions[action]);
    console.log('action: ' + this.actions[action]);
  }

   for (var i = 0, len = this.knowledge.length; i < len; i++ ) {
    var knowledge = this.knowledge[i]
    for (var obj in knowledge) {
      console.log("knowledge: " + obj);
      this.classifier.addDocument(obj, obj);
    }
  }
